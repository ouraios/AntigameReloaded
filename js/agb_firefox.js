...
AGB.Manager = {
  Messages: function(a, b, c, d, e) {
    a &&
      OBJ.is(AGB[a]) &&
      "function" === typeof AGB[a].Messages &&
      ("function" === typeof d
        ? AGB[a].Messages(
            b,
            JSON.parse(c || "{}"),
  ...
  Start: function() {
    ...
    try {
      AGB.Core.Services.scriptloader.loadSubScript(
        AGB.Config.path + "js/api_firefox.js",
        AGB.API.prototype,
        "UTF-8"
      );
    } catch (b) {
  ...
  Stop: function(a) {
    ...
    AGB.status = 0;
    for (b in AGB)
      AGB.hasOwnProperty(b) &&
        "object" === typeof AGB[b] &&
        "Config" !== b &&
        "Core" !== b &&
        "Manager" !== b &&
        "Storage" !== b &&
        (AGB[b].Data && (AGB[b].Data = null),
        AGB[b].Player && (AGB[b].Player = null),
        AGB[b].Status && (AGB[b].Status = null),
        (AGB[b] = null));
  },
  WindowListener: {
  ...
  Check: function(a, b, c) {
    ...
    !a.AGO_started &&
      ...
      a.top === a.self &&
      c &&
      "content-document-global-created" === b &&
  ...
  Load: function(a, b) {
    ...
    3 === a.mode
      ...
      : 2 === a.mode
        ? ((c = ["js/basics.js", "js/common.js", "js/init.js"]),
          2 <= AGB.Config.beta && c.unshift("js/constant.js"))
        : (c = ["js/coordinates.js"]);
  ...
  message: function(a, b, c, d) {
    ...
    if ((f = AGB.App.getPlayer(a)))
      try {
        for (
          l = Components.classes["@mozilla.org/appshell/window-mediator;1"]
            .getService(Components.interfaces.nsIWindowMediator)
            .getEnumerator("navigator:browser");
          l.hasMoreElements();

        )
          if ((g = l.getNext().gBrowser) && g.browsers)
            for (n = g.browsers.length, k = 0; k < n; k++)
              (h = g.getBrowserAtIndex(k)),
                -1 <
                  (h.currentURI.spec || "").indexOf("ogame.gameforge.com/") &&
                  (e = h.contentDocument.getElementById("ago_global_data")) &&
                  e.getAttribute("ago-data-key") === f &&
                  ((a = {
...
AGB.Storage = {
  ...
  Start: function(a) {
    ...
    AGB.Storage.local = AGB.Core.Services.prefs.getBranch(
      "extensions." + AGB.Config.id + "."
    );
    AGB.Storage.sync = AGB.Core.Services.prefs.getBranch(
      "services.sync.prefs.sync.extensions." + AGB.Config.id + "."
    );
    if (AGB.Storage.local) {
      b = Math.floor(Date.now() / 1e3);
      try {
        AGB.Storage.local.setIntPref("App_Start", b),
          (AGB.Storage.status =
            b === +AGB.Storage.local.getIntPref("App_Start") ? 1 : 0);
      } catch (c) {}
    }
    a();
  },
  Set: function(a, b) {
    ...
    if (
      OBJ.is(a) &&
      (a.key ? ((c = {}), (c[a.key] = a.data)) : (c = a.data),
      OBJ.is(c) && Object.keys(c).length)
    )
      try {
        OBJ.iterate(c, function(a) {
          ...
          AGB.Storage.local.setComplexValue(
            a,
    ...
    function c(a) {
      ...
      try {
        ...
        if (b === Components.interfaces.nsIPrefBranch.PREF_STRING)
          return AGB.Storage.local.getComplexValue(
            a,
            Components.interfaces.nsISupportsString
          ).data;
