...
AGB.App = {
    ...
    "Start" === a
      ...
      : "Update" === a
        ...
        : "Refresh" === a
          ...
          : "Script" === a &&
            OBJ.is(b) &&
            AGB.Manager.loadScripts(b.scripts, d);
  },
  Check: function(a) {
    var b, c;
    ...
    return a
      ? ((b = { href: a, host: c }),
        a.match(
          ...
          ? (b.mode = 3)
          : OBJ.iterate(AGB.App.Extern, function(a) {
  ...
  Start: function(a, b, c) {
    ...
    OBJ.is(a) &&
      a.page &&
      b &&
      (AGB.Manager.loadScripts(
        OBJ.get(AGB.App.Page[a.page], "js") || ["pages"],
        c
      ),
      2 === a.mode
        ? b({
        ...
        : 3 <= a.mode &&
          ...
          a.keyUni &&
          a.keyPlayer &&
          AGB.Com.Check(a.abbrCom) &&
          a.abbrUni &&
          a.urlUni &&
          ((d = a.keyPlayer),
          OBJ.is(AGB.App.Player[a.keyUni]) ||
            (AGB.App.Player[a.keyUni] = {
              abbrUni: a.abbrUni,
              urlUni: a.urlUni
            }),
          (AGB.App.Player[a.keyUni].keyPlayer = d),
          OBJ.is(AGB.App.Player[d]) ||
            (AGB.App.Player[d] = {
          ...
          (a.reload = a.reload || 1 !== AGB.App.Player[d].status),
          (AGB.App.Player[d].status = 1),
          AGB.Core.clearTimeout(AGB.App.Player[d].timeout),
          (AGB.App.Player[d].timeout = AGB.Core.setTimeout(function() {
  ...
  Update: function(a, b) {
    ...
    (c = AGB.App.getPlayer(a, "copy")) &&
      (a.reload ||
        2e3 <
          AGB.Time.timestamp() - (+AGB.App.Player[c].timestampUpdate || 0)) &&
      ((AGB.App.Player[c].timestampUpdate = AGB.Time.timestamp()),
      AGB.App.Load(a),
      AGB.Uni.Load(a),
      AGB.Label.Load(a),
      AGB.DataBase.Init(a),
  ...
  Init: function(a, b) {
    ...
    if ((d = AGB.App.getPlayer(a, "copy")))
      (c = OBJ.parse(b[AGB.Data.getKey(d, "App", "Data")])),
        (AGB.App.Data[d] = c.version === g ? c : { version: g }),
        (AGB.App.Data[d].storage = AGB.Storage.status);
  },
  Load: function(a) {
    var b, c;
    if ((c = AGB.App.getPlayer(a, "copy")))
      (b = new XMLHttpRequest()),
        ...
        (b.onerror = b.onload = function() {
          ...
          g &&
            c &&
            ((c.storage = AGB.Storage.status),
            (e = OBJ.parse(200 === +b.status ? b.responseText : "")),
            e.versionFinal &&
              ((c.versionLoca = e.versionLoca),
              (c.versionLocaMenu = e.versionLocaMenu),
              (c.versionUpdate =
                1 < AGB.Config.beta
                  ? ""
                  : 1 === AGB.Config.beta
        ...
        b.send(
          "domain=antigame.de&loca=" +
            a.abbrCom +
            "&locamenu=" +
            (AGB.Option.Get(c, "A10") || a.abbrCom)
        );
  },
  Save: function(a) {
    a = AGB.App.getPlayer(a);
    AGB.Data.isStorage(a, "App", "Data") &&
      OBJ.is(AGB.App.Data[a]) &&
      AGB.Data.setStorage(a, "App", "Data", AGB.App.Data[a]);
  },
  Get: function(a, b, c) {
    var d;
  ...
  Set: function(a, b, c, d) {
    a &&
      ...
      c &&
      AGB.App.Data[a][c] !== d &&
      ((AGB.App.Data[a][c] = d), AGB.App.Save({ player: a }));
  },
  getUni: function(a) {
    return AGB.status && a && a.keyUni && AGB.App.Player[a.keyUni]
      ? ((a.keyPlayer = AGB.App.Player[a.keyUni].keyPlayer),
        (a.abbrUni = AGB.App.Player[a.keyUni].abbrUni),
        (a.urlUni = AGB.App.Player[a.keyUni].urlUni),
        a.keyUni)
      : "";
  },
...
AGB.Data = {
  ...
  Init: function(a, b) {
    ...
    (!AGB.Data.Status[c] || (a.reload && 1 === AGB.Data.Status[c]))
      ? ((AGB.Data.Status[c] = 3),
        (d = {}),
        OBJ.iterate(AGB.Data.Info, function(b) {
          OBJ.iterate(AGB.Data.Info[b], function(e) {
            var f;
            1 <= AGB.Data.Info[b][e].storage &&
              ((f =
                2 === AGB.Data.Info[b][e].key
                  ? a.keyCom
                  : 1 === AGB.Data.Info[b][e].key
                    ? a.keyUni
                    : c),
              (d[AGB.Data.getKey(f, b, e)] = e));
          });
        }),
    ...
    function c(a) {
      ...
      AGB.Data.isStatus(a) &&
        ...
        OBJ.iterate(AGB.Data.Info, function(a) {
          AGB[a] &&
            "function" === typeof AGB[a].Save &&
            "App" !== a &&
            AGB[a].Save(c);
        }),
        b
          ? AGB.Storage.Set({ data: c.save }, function() {
              b(c.backup);
            })
  ...
  Sync: function(a) {
    ...
    AGB.Data.isStatus(b) &&
      ((d = Boolean(
        AGB.Option.Get(b, "D60") && 3 === AGB.Option.Get(b, "D61")
      )),
      (c = {}),
      (c[b + "_SYNC_Sync_Data"] = d),
      AGB.Data.iterate("", function(a, e) {
        2 <= AGB.Data.get(a, e, "storage") &&
          (c[b + "_SYNC_" + a + "_" + e] = d);
      }),
      AGB.Storage.Sync({
        sync: !0,
  ...
  Backup: function(a, b) {
    function c(a, b) {
      ...
      d =
        ...
        "&ident=" +
        a.ident +
        "&type=Sync&action=put&domain=antigame.de&string=" +
      ...
      c.onerror = c.onload = function() {
        b &&
          ((a.status = 200 === +c.status && 7 <= +c.responseText ? 1 : -1),
          (a.error = -1 === a.status),
          (a.data = null),
          a.error || AGB.App.Set(a.keyPlayer, "timestampSync", a.timestamp),
          b(a));
      };
      c.send(d);
    }
    ...
    AGB.Data.isStatus(g) &&
      ((e = a.tab),
      (h = OBJ.create(a)),
      (h.timestamp = AGB.Time.timestamp()),
      (h.list = {}),
      (h.data = {}),
      (h.timestampLocal = AGB.App.Get(g, "timestampSync")),
      (h.data.Sync_Data = {
        com: a.abbrCom,
        uni: a.abbrUni,
        player: a.accountId,
      ...
      (f = 3 === e ? 4e3 : 2 === e ? 1e4 : 3e5),
      AGB.Data.Save(a, function(a) {
        OBJ.iterate(a, function(b) {
          var c;
          c = OBJ.parse(a[b]);
          STR.check(a[b]).length > f
            ? ((h.data[b] = { version: 0 }),
              (h.data.Sync_Data[b] = 1),
              (h.list[b] = -1))
            : ((h.data[b] = c.version ? c : { version: 1 }),
              (h.data.Sync_Data[b] = STR.hash(JSON.stringify(h.data[b]))),
              (h.list[b] = 1));
        });
        1 === e ? b && b(h) : 2 === e ? c(h, b) : 3 === e && d(h, b);
      }));
  },
  Restore: function(a, b) {
      ...
      if (OBJ.is(c) && !c.error) {
        ...
        if (1 === a.tab)
          -1 === e.com && (a.type = Math.max(a.type, 3)),
            -1 === e.uni && (a.type = Math.max(a.type, 4)),
            -1 === e.player && (a.type = 9);
        else if (1 !== e.com || 1 !== e.uni || 1 !== e.player) a.type = 0;
        2 <= a.type
          ? AGB.Data.Save(a, function(d) {
              AGB.Data.iterate("", function(b, g) {
                var f;
                AGB.Data.get(b, g, "storage") >= a.type &&
                  ((f = b + "_" + g),
                  (e.list[f] = c[f]
                    ? 1 === c[f]
                      ? -1
      ...
      if (OBJ.is(c) && !c.error && OBJ.is(c.Sync_Data)) {
        ...
        if (1 === a.tab)
          -1 === f.com && (a.type = Math.max(a.type, 3)),
            ...
            -1 === f.player && (a.type = 9);
        else if (1 !== f.com || 1 !== f.uni || 1 !== f.player) a.type = 9;
        AGB.Data.iterate("", function(b, d) {
          var e;
          e = b + "_" + d;
          AGB.Data.get(b, d, "storage") >= a.type &&
            1 <= +OBJ.get(c[e], "version") &&
            ((f.list[e] = 1), (f.data[g + "_" + e] = JSON.stringify(c[e])));
        });
          ...
          ? AGB.Storage.Set({ data: f.data }, function(a) {
              1 === a &&
                AGB.App.Set(
                  g,
                  "timestampSync",
                  +f.timestamp || AGB.Time.timestamp()
                );
              AGB.Data.Status[g] = 0;
              f.status = a;
              f.data = null;
    ...
    AGB.Data.isStatus(g) &&
      (1 === a.tab
        ? "restore" === a.action
          ? d(OBJ.parse(a.value), a.universal)
          : c(OBJ.parse(a.value).Sync_Data, a.universal)
        : 2 === a.tab
          ? ((f =
              "com=" +
              a.abbrCom +
          ...
          : 3 === a.tab &&
            ((f = {}),
            (f[g + "_SYNC_Sync_Data"] = "Sync_Data"),
            AGB.Data.iterate("", function(a, b) {
  ...
  removeStorageGroup: function(a, b) {
    OBJ.is(a) &&
      AGB.Data.Info[b] &&
      AGB.Data.iterate(b, function(c, d) {
        var g;
        1 <= c.storage &&
          ((g = 2 === c.key ? a.keyCom : 1 === c.key ? a.keyUni : a.keyPlayer),
          AGB.Storage.Remove({ key: AGB.Data.getKey(g, b, d) }));
      });
  },
  setStorage: function(a, b, c, d) {
    a &&
      ((d = OBJ.is(d) ? JSON.stringify(d) : d || ""),
      AGB.Storage.Set({ key: AGB.Data.getKey(a, b, c), data: d }));
  },
    ...
    if (AGB.Data.Info[a])
      ...
      for (a in AGB.Data.Info)
        if (AGB.Data.Info.hasOwnProperty(a))
          for (c in AGB.Data.Info[a])
            AGB.Data.Info[a].hasOwnProperty(c) && b(a, c);
  },
  getKey: function(a, b, c) {
    return AGB.Data.Info[b] && AGB.Data.Info[b][c] ? a + "_" + b + "_" + c : "";
  },
  get: function(a, b, c, d) {
    return AGB.Data.Info[a] && AGB.Data.Info[a][b]
      ? 6 === d
        ? STR.check(AGB.Data.Info[a][b][c] || "")
        : +AGB.Data.Info[a][b][c] || 0
      : 6 === d
        ? ""
        : 0;
  },
  getTab: function(a, b) {
    return a &&
      a.tab &&
      AGB.Data.Info[b] &&
      AGB.Data.Info[b][a.tab] &&
      AGB.Data.Info[b][a.tab].tab
      ? a.tab
      : "";
  },
  set: function(a, b, c, d) {
...
AGB.Uni = {
  ...
  Init: function(a, b) {
    ...
    if ((d = AGB.App.getPlayer(a)))
      (AGB.Uni.Data[d] = OBJ.create(AGB.Uni.Info)),
        (AGB.Uni.Data[d].version = g),
        (c = OBJ.parse(b[AGB.Data.getKey(d, "Uni", "Data")])),
        c.version === g && OBJ.copy(c, AGB.Uni.Data[d]);
  },
  Save: function(a) {
    a = AGB.App.getPlayer(a);
    AGB.Data.isStatus(a) &&
      OBJ.is(AGB.Uni.Data[a]) &&
      AGB.Data.setStorage(a, "Uni", "Data", AGB.Uni.Data[a]);
  },
  Load: function(a) {
    var b;
    AGB.App.getPlayer(a, "copy") &&
      ...
      (b.onerror = b.onload = function() {
        ...
        d &&
          ...
          (200 === +b.status &&
            b.responseText &&
            OBJ.iterate(AGB.Uni.Info, function(a) {
              var d;
              ...
              2 === d.length &&
                ...
                (d = (d[1] || "").split("<")[0]),
                (d = (d || "").trim()),
                (c[a] = "number" === typeof AGB.Uni.Info[a] ? +d || 0 : d));
            }),
