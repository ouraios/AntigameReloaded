...
AGB.Manager = {
  ...
  Check: function(a, c, b) {
    OBJ.is(b) &&
      OBJ.is(c) &&
      "loading" === c.status &&
      ((c = AGB.App.Check(b.url)),
      OBJ.is(c) && c.mode && AGB.Manager.Load(c, a));
  },
  Load: function(a, c) {
    1 === a.mode &&
  ...
  message: function(a, c, b, d) {
    ...
    (f = AGB.App.getPlayer(a)) &&
      chrome.tabs.query({ url: "https://*.ogame.gameforge.com/*" }, function(
        a
      ) {
        var e;
        for (e = 0; e < a.length; e++)
          a[e] &&
            a[e].id &&
            chrome.tabs.sendMessage(a[e].id, {
              player: f,
              page: c,
              role: b,
              data: d
...
chrome.runtime.onMessage.addListener(function(a, c, b) {
  ...
  if (
    (c = "object" === typeof c && c.tab ? c.tab.id : "") &&
    "object" === typeof a &&
    ("Log" === a.page
      ? window.console.log("AntiGameOrigin:  " + a.para)
      : "Storage" === a.page
        ? "Set" === a.role
          ? AGB.Storage.Set(a.para)
          : "Get" === a.role
            ? AGB.Storage.Get(a.para, b)
            : "Remove" === a.role
              ? AGB.Storage.Remove(a.para)
              : "RemoveFilter" === a.role && AGB.Storage.RemoveFilter(a.para, b)
        : "Update" === a.page
          ? "Check" === a.role &&
            chrome.runtime.requestUpdateCheck(function(c) {
              AGB.Manager.message(a.para, "Menu", "Install", c);
            })
          : a.page &&
            OBJ.is(AGB[a.page]) &&
            "function" === typeof AGB[a.page].Messages &&
  ...
  Start: function(a) {
    ...
    chrome.storage && chrome.storage.local
      ? ((c = Math.floor(Date.now() / 1e3)),
        chrome.storage.local.set({ App_Start: c }, function() {
