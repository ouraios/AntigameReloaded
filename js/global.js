window.addEventListener(
  ...
  function(b) {
    ...
    switch (a.role) {
      ...
      case "getProperty":
        a.property &&
          AGO.setData(a.property, JSON.stringify(window[a.property]));
        break;
      case "sendShips":
        window.sendShips(
          a.mission,
      ...
      case "maximalSpeed":
        ...
        do
          if (
            ((window.duration = window.getDuration()),
            (window.consumption = window.getConsumption()),
            1 < window.speed &&
              (window.consumption > window.storageCapacity ||
                window.consumption > window.currentDeuterium))
          )
            window.speed--;
          else break;
        while (1);
...
var AGO = {
  ...
  message: function(b, a, c) {
    window.dispatchEvent(
      new window.CustomEvent("ago_global_send", {
  ...
  getData: function(b) {
    ...
    if (a) return a.getAttribute("ago-data-" + b) || "";
  },
  Interactive: function(b) {
    ...
    if ((a = 1 < AGO.Data.timeStatus ? AGO.Data.timeZoneDelta : 0))
      (c = window.getFormatedDate),
        (window.getFormatedDate = function(b, e) {
          return c(b + a, e);
        });
    ...
    window.toggleMaxShips = function(a, b, c) {
      var k;
      if (
        ("#shipsChosen" === a && AGO.Data.F00) ||
        ("#jumpgateForm" === a && AGO.Data.G30)
      )
    ...
    window.sendShips = function(a, b, c, d, h, l, m) {
      ...
      m = m || "";
      1 === +window.shipsendingDone &&
        ((window.shipsendingDone = 0),
        (c = {
          mission: a,
          galaxy: b,
          system: c,
  ...
  Ready: function() {
    $(document).ready(function() {
      AGO.message("Init", "Ready");
      window.timerHandler &&
        window.timerHandler.appendCallback(function() {
          AGO.message("Init", "Timer");
        });
    });
    $(document).ajaxSuccess(function(b, a, c) {
      b = decodeURIComponent(c.url || "")
        .replace(/\?/g, "&")
        .split("&page=")[1];
      b = (b || "")
        .toLowerCase()
      ...
      AGO.message("Init", "Content", {
        ...
        para: c.data,
        response:
          "fetcheventbox" === b || "minifleet" === b ? a.responseText || "" : ""
      });
    });
  },
  Tooltip: function() {
                ...
                : k &&
                  ("overview" === g
                    ...
                    : "resources" === g ||
                      ...
                      : "fleet1" === g
                        ? -1 <
                            ($(c).attr("onclick") || "").indexOf(
                              "toggleMaxShips"
                            ) && (h = 1 === AGO.Data.F02 || 3 === AGO.Data.F02)
                        : "fleet2" !== g &&
                          "fleet2" !== g &&
                          "movement" !== g &&
                          ("galaxy" === g
  ...
  Jumpgate: function() {
    ...
    window.jumpToTarget = function() {
      AGO.Data.G34
        ? (document.location.href =
            document.location.href.split("?")[0] + "?page=fleet1")
        : b();
    };
    ...
    window.jumpgateDone = function(b) {
      ...
      d.status && AGO.Data.G33
        ? ((d.errorbox = {}), a(JSON.stringify(d)), window.jumpToTarget())
        : a(b);
    };
  },
  Fleet: function() {
    if ("fleet1" === AGO.Data.page) {
      ...
      window.setTType = function(a) {
        if (a) {
          try {
            AGO.setData("type", a),
              AGO.message("Page", "clickType"),
              (a = +AGO.getData("type") || 0);
          } catch (b) {}
          a && d(a);
        } else d();
      };
      ...
      window.checkRessourceByType = function(a) {
        window[a + "OnPlanet"] = parseInt(
          ($("#resources_" + a).text() || "").replace(/[^\d\-]/g, ""),
          10
        );
        g(a);
      };
