AGO.Websim = {
  ...
  Interactive: function() {
    AGO.Websim.isAntigame = Boolean(
      "antigame" === STR.getParameter("ref", document.location.href)
    );
    for (var b = 0; 16 > b; b++)
      AGO.Websim.Data[b] = {
        attacker: 0,
  ...
  Show: function() {
    ...
    c &&
      ...
      DOM.set(c.parentNode.querySelectorAll("input")[1], null, null, {
        padding: "3px",
        margin: "0px 10px",
        minWidth: "150px"
      }));
    if ((a = document.getElementById("del_techs"))) {
      AGO.Websim.isAntigame &&
        (a.checked = STR.getParameter("del_techs", document.location.href));
      var d =
          NMR.parseInt(STR.getParameter("uni_speed", document.location.href)) ||
          1,
        c = document.createElement("tr");
      c.innerHTML =
        "<td>" +
        AGO.Label.get("X04") +
        '</td><td><input id="uni_speed" size="3" maxlength="2" value="' +
        d +
        '"></td>';
    ...
    c.innerHTML =
      ...
      AGO.Label.get("X02") +
      '</td><td align="center"><div id="anti_units_attacker" style="width:50px; text-align: left; padding: 5px 10px;"></div></td><td align="left" id="anti_retreat_ratio" style="width:50px; white-space: nowrap;"></td><td align="center"><div id="anti_units_defender" style="width:50px; text-align: left; padding: 5px 10px;"></div></td><td></td>';
    (a = document.querySelector("#shiptable tbody")) &&
      a.insertBefore(c, a.childNodes[1]);
    for (c = 0; 13 >= c; c++) b("a", c);
    for (c = 0; 13 >= c; c++) b("d", c);
  ...
  ShowTitle: function() {
    var b = "";
    AGO.Websim.isAntigame &&
      ((b = STR.getParameter("enemy_status", document.location.href)),
      (b =
        "honorableTarget" === b
          ? "#FFFF66"
          : "inactive" === b
      ...
      (b =
        ...
        STR.getParameter("uni", document.location.href) +
        "</span>" +
        STR.getParameter("enemy_name", document.location.href) +
        ' <span style="color: white;">[' +
        STR.getParameter("enemy_pos", document.location.href) +
        ']</span> -  <span style="color: ' +
        b +
        ';">' +
        STR.getParameter("enemy_player", document.location.href) +
        "</span> - " +
        decodeURI(STR.getParameter("report_time", document.location.href))));
    var b =
        b +
        ('<a href="javascript:void(0)" id="ago_routine_harvest" style="float: right; background-color: #344566; border: 1px solid #415680; padding: 3px; margin: 0 10px;" title=" ' +
          AGO.Label.get("X01") +
          '">AGO ' +
          AGO.Label.get("F05") +
          "  " +
      ...
      b =
        b +
        ('<a href="javascript:void(0)" id="ago_routine_attack" style="float: right; background-color: #344566; border: 1px solid #415680; padding: 3px;" title=" ' +
          AGO.Label.get("X01") +
          '">AGO ' +
          AGO.Label.get("F05") +
          "  " +
    ...
    a &&
      ((DOM.append(a.parentNode, "span", null, {
        color: "#6F9FC8",
        fontSize: "14px"
      }).innerHTML = b),
      DOM.set("ago_routine_attack", "id", null, null, {
        click: AGO.Websim.SetAttack
      }),
      DOM.set("ago_routine_harvest", "id", null, null, {
        click: AGO.Websim.SetHarvest
      }));
  },
  ShowTarget: function() {
    function b(a) {
    ...
    function a(a, b) {
      return (
        '<input type="radio" name="plunder_ratio" value="' +
        a +
        '"' +
        (a === b ? ' checked="checked"' : "") +
        ">" +
    ...
    if (d) {
      ...
      c.type = "checkbox";
      c.checked = Boolean(
        "3" === STR.getParameter("enemy_type", document.location.href)
      );
      d.parentNode.appendChild(document.createTextNode(" M"));
    }
    DOM.addEvents("input[name=plunder_ratio]", null, {
      click: AGO.Websim.CalculateResourse
    });
    b("enemy_metal");
    b("enemy_crystal");
    b("enemy_deut");
    AGO.Websim.CalculateResourse();
    DOM.set("#read_field + input", null, null, null, {
      click: AGO.Websim.ReadResourse
    });
  ...
  ShowResult: function(b) {
    var a,
      c = document.getElementById("result_table");
    if (c) {
      DOM.removeObserver(AGO.Websim.ObserverObject);
      var d = c.querySelector("table");
      d.style.display = "none";
      for (
        var e = d.getElementsByTagName("tr"), d = [], n, p, q, h = 0;
        h < e.length;
        h++
      ) {
        var r = e[h].firstChild.innerHTML,
          m = e[h].lastChild.innerHTML,
          f,
          g = h,
          k = m;
        f = {};
        ...
        2 === l.length
          ? ((f.flag = !0),
            ...
            (g = l[1].split(" ~ ")),
            2 === g.length && ((l[1] = g[0]), (f.info = "~ " + g[1])),
            (g = k.split(" (")),
            2 === g.length && ((l[1] = g[0]), (f.info = g[1].split(")")[0])),
            3 === g.length &&
              ((l[1] = ""),
              (f.metal_perc = g[1].split(")")[0]),
              (f.crystal_perc = g[2].split(")")[0]),
              (n = g[0].split(" ")[1]),
              (p = g[1].split(" ")[2])),
            (k = l[1].split(" ")),
          ...
          : 8 === g &&
            ...
            (q = k.split(" ")[1]));
        k =
          '<td style="min-width: 80px; text-align: right; padding: 0px 6px; color: ' +
          (6 === h || 7 === h
            ? "#FF9600"
            : 2 === h
              ? "#008000"
        ...
        f.flag && !b
          ? (2 === h && (a = OBJ.create(f)),
            (m =
              ...
              STR.check(f.metal) +
              '</td><td width="30">' +
              STR.check(f.metal_perc) +
              "</td>" +
              k +
              STR.check(f.crystal) +
              '</td><td width="30">' +
              STR.check(f.crystal_perc) +
              "</td>" +
              k +
              STR.check(f.deuterium) +
              '</td><td style="min-width: 80px; padding: 0px 8px; white-space: nowrap;">' +
              STR.check(f.info) +
              "</td>"))
          : (9 === h &&
              ((f = NMR.minMax(
                NMR.parseInt(document.getElementById("uni_speed").value),
                1,
                100
              )),
              (m =
                AGO.Time.formatTime(Math.ceil(AGO.Time.parseTime(m) / f)) +
                " h")),
            (m = '<td colspan="6" width="400">' + (b ? "" : m) + "</td>"));
        d.push(
          "<tr" +
            ("none" === e[h].style.display ? ' style="display: none;"' : "") +
            '><td style="padding: 2px 15px 2px 5px; white-space: nowrap;">' +
            r +
            "</td>" +
            m +
            "</tr>"
      ...
      b ||
        ((b = c.appendChild(document.createElement("table"))),
        (b.id = "anti_result_table"));
      a = {
        metal_debris: OBJ.get(a, "metal"),
        crystal_debris: OBJ.get(a, "crystal")
      };
      DOM.setData(b, null, a);
      a = d[3];
      d[3] =
        '<tr><td style="text-align: right; padding: 10px 6px 2px 5px;" colspan="2">' +
        n +
        '</td><td style="text-align: right; padding: 10px 6px 2px 5px;" colspan="2">' +
        p +
        '</td><td style="text-align: right; padding: 10px 6px 2px 5px;" colspan="2">' +
        q +
        "</td><td></td></tr>" +
  ...
  Observer: function(b) {
    for (var a in b)
      b[a].addedNodes &&
        "result_table" === b[a].target.id &&
        AGO.Websim.ShowResult();
  },
  CalculateFleet: function() {
    ...
    var a,
      c,
      d,
      e = document.getElementById("acs").selectedIndex;
    AGO.Websim.Data[e] = {
      attacker: 0,
      defender: 0,
      count_attacker: 0,
    ...
    for (a = 0; 13 >= a; a++)
      if ((c = document.getElementsByName("ship_a_" + a + "_b")[0]))
        (d = NMR.parseInt(c.value)),
          (c.value = STR.check(d)),
          0 < d &&
            ((AGO.Websim.Data[e].count_attacker += d),
            (AGO.Websim.Data[e].attacker +=
              AGO.Item[AGO.Websim.ships[a]].retreat * d));
    for (a = 0; 13 >= a; a++)
      if ((c = document.getElementsByName("ship_d_" + a + "_b")[0]))
        (d = NMR.parseInt(c.value)),
          (c.value = STR.check(d)),
          0 < d &&
            (13 >= a && (AGO.Websim.Data[e].count_defender += d),
            (AGO.Websim.Data[e].defender +=
              AGO.Item[AGO.Websim.ships[a]].retreat * d));
    var n = (e = d = c = 0);
    for (a = 0; 16 > a; a++)
      (c += +AGO.Websim.Data[a].attacker || 0),
        (d += +AGO.Websim.Data[a].defender || 0),
        (e += +AGO.Websim.Data[a].count_attacker || 0),
        (n += +AGO.Websim.Data[a].count_defender || 0);
    document.getElementById("anti_units_attacker").innerHTML = STR.check(e);
    ...
    c &&
      ...
      (c > d
        ? ((e = b(c / d) + "&nbsp; : &nbsp;1"),
          (a = c < 3 * d ? "#008000" : c < 5 * d ? "#FFFF00" : "#FF4B00"))
        : ((e = "1&nbsp; : &nbsp;" + b(d / c)), (a = "#FF0000")));
  ...
  SetAttack: function() {
    ...
    b.mission = 1;
    b.routine = 3;
    b.metal = DOM.getValue('input[name="original_enemy_metal"]', null, 3);
...
AGO.Osimulate = {
  ...
  SetAttack: function() {
    ...
    b.deuterium = DOM.getValue("input#numDeuterio", null, 3);
  ...
  SetHarvest: function() {
    ...
    if ((b = document.getElementById("tdMediaDetriti")))
      (c = STR.check(b.innerHTML)),
        (b = (c.split("locMetal2")[1] || "").split("<span")[0]),
        (a.metal = NMR.parseIntAbs(c.split("<")[0])),
